...
class  VedioDownLoadProcesser:
    ...
    def tsDownload1By1(self, url, mp4Name):
        ...
        for index, line in enumerate(all_line): # 第二层
            ...
            if "#EXT-X-KEY" in line:  # 找解密Key
                ...
                print(f'the self.key')
                
                print(f'the self.key {self.key}')
                if not self.key:
                   print("2322323")
                   uri_pos = line.find("URI")
                   quotation_mark_pos = line.rfind('"')
                   print(line[uri_pos:quotation_mark_pos])
    ...
    def downLoadStart(self, url, savePath, mp4Name = None):
        #创建下载路径等准备
        self.init(savePath)
        self.downSuccess = 0
        self.downTotal = 0
